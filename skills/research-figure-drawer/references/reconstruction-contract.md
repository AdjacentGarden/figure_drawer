...
Source: https://github.com/ningzimu/image-to-editable-ppt-skill -> skills/image-to-editable-ppt/SKILL.md
Commit: b7be494e31a0ed56ef98716891db5474606b8cdf (2026-09-16T14:41:10+08:00)
License: MIT, Copyright (c) 2026 ningzimu (full text: ../cli/LICENSE)
Integrity: ../cli/VENDOR.json
-->

# Reconstruction contract (vendored from image-to-editable-ppt)

This is the authoritative contract for step 5-6 of this skill: the `editppt` state machine,
the page-worker ownership boundary, and the deck packaging rules. The runtime it drives is
bundled in this skill at `<skill-root>/cli`, so no second skill has to be installed.
Where this contract says "the `image-to-editable-ppt` skill", read "this skill's bundled
reconstruction contract". Where it or its references say `<skill-root>`, read "this skill's
directory" (the one containing `SKILL.md` and the bundled `cli/`); the vendored CLI therefore
sits at `<skill-root>/cli` exactly as those commands expect.
Setup or update commands embedded in the vendored body below (for example the
`npx skills add ...` line under "Updating This Skill") are upstream self-setup text and must
NOT be executed here: this skill already contains the runtime. To change vendored versions,
follow `installation.md` in this skill and re-vendor instead.
Rules that are specific to figures authored from research text or
TeX (object-source policy) live in [hybrid-reconstruction.md](hybrid-reconstruction.md) and
override the generic screenshot-fidelity rule where they conflict.
